...
export async function GET() {
  ...
  const { data } = await supabase
    .from("conversations")
    .select(`*, p1:profiles!conversations_participant_1_fkey(id,username,full_name,avatar_url,last_seen_at), p2:profiles!conversations_participant_2_fkey(id,username,full_name,avatar_url,last_seen_at)`)
...
export async function POST(req: NextRequest) {
  ...
  // Valider que otherUserId est un UUID valide pour éviter l'injection de filtre PostgREST
